...
def removeElemFromMapByValue(map_to_update, value):
    ...
    map_to_update = {key:val for key, val in map_to_update.items() if val != value}
...
with open(filename, 'a') as out:
    for idx,inter in intersection_map.items():
        if (len(inter.time_map) > 0):            
            ...
            time_street_map = {key:val[1] for key, val in inter.time_map.items()}
...
#             print('Time street map len: ' + str(len(time_street_map)))
            while (stillSomeStreetsLeft and current_offset != 0):
                ...
                time_street_map = removeElemFromMapByValue(time_street_map, most_recurring_street)
